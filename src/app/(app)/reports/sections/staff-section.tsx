...
export async function StaffSection({ month }: { month?: string }) {
  const months = recentMonthOptions(12);
  const { year, monthIndex, value: selectedMonth } = parseMonthValue(month);
    ...
    <div className="space-y-6">
      <div className="no-print flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-muted">
          Per-cashier sales for the selected month — revenue, items, average
          sale, and best-selling product.
        </p>
      ...
      {/* Month picker stays on the same /reports route, just nudges the param. */}
      <form
        method="get"
        action="/reports"
      ...
      <section className="grid gap-3 grid-cols-2 md:grid-cols-4">
        <Stat
          label="Total revenue"
          value={`Rs ${formatAmount(totals.revenue.toFixed(2))}`}
          accent
        />
