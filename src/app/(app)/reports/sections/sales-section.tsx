...
export async function SalesSection({ range: rawRange }: { range?: string }) {
    ...
    <div className="space-y-8">
      <div className="no-print flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-muted">
          Track revenue, best-sellers, and stock health. Switch period below —
...
function RangeToggle({ current }: { current: ReportRange }) {
  ...
  return (
    <div
      ...
      className="no-print inline-flex rounded-lg border border-white/10 bg-card p-1 text-sm"
    >
      {ranges.map((r) => {
        ...
        return (
          <Link
            ...
            href={{ pathname: "/reports", query: { section: "sales", range: r } }}
