    ...
    <div className="space-y-8">
      <div className="no-print flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-ink-muted">
          Gross profit over the last 30 days and the value tied up in on-hand
          stock. COGS uses the current product cost — drift is small for a
          single-shop catalog.
        </p>
        <PrintButton label="Print profit" />
      </div>

      <section className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <StatCard
          label="Revenue · 30d"
          value={formatAmount(windowTotals.revenue.toFixed(2))}
        />
        <StatCard
          label="Cost of goods · 30d"
          value={formatAmount(windowTotals.cost.toFixed(2))}
        />
        <StatCard
      ...
      <section>
        <div className="mb-3 flex items-end justify-between">
          ...
          <p className="text-xs text-ink-muted">
            {stockValue.totalSkus} active SKU
            {stockValue.totalSkus === 1 ? "" : "s"} · {stockValue.totalUnits}{" "}
            units
          </p>
        </div>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <StatCard
            label="At retail"
            value={formatAmount(stockValue.totalRetail)}
          />
          <StatCard
        ...
        <div className="mt-4 overflow-x-auto rounded-lg border border-white/10 bg-card">
          <table className="min-w-full divide-y divide-white/10 text-sm">
            <thead className="bg-surface text-left text-xs uppercase tracking-wider text-ink-muted">
              <tr>
